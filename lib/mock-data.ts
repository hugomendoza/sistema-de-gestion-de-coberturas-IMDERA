...
// Función para obtener la fecha de la próxima semana
const getNextWeekDates = () => {
  const today = new Date();
  const nextWeek = new Date(today);
  nextWeek.setDate(today.getDate() + 7);
  return {
    start: new Date(today.setDate(today.getDate() - today.getDay() + 1)), // Lunes de esta semana
    end: new Date(nextWeek.setDate(nextWeek.getDate() - nextWeek.getDay() + 7)), // Domingo de la próxima semana
  };
};
...
const instructors = [
  { id: "1", name: "Carlos Mendoza", specialty: "Emprendimiento" },
  { id: "2", name: "Ana Torres", specialty: "Finanzas" },
  { id: "3", name: "Luisa Ramírez", specialty: "Marketing" },
  { id: "4", name: "Jorge Herrera", specialty: "Desarrollo" },
  { id: "5", name: "María Gómez", specialty: "Liderazgo" },
];
...
export const mockProject: Project = {
  ...
  description:
    "Fortalecimiento de capacidades empresariales en comunidades rurales del departamento",
  classes: mockClasses,
  totalStudents: 80,
};
...
export const mockStatistics: Statistics = {
  ...
  genderDistribution: {
    male:
      (mockStudents.filter((s) => s.gender === "masculino").length /
        mockStudents.length) *
      100,
    female:
      (mockStudents.filter((s) => s.gender === "femenino").length /
        mockStudents.length) *
      100,
    other: 0,
  },
  ...
  overweightMetrics: {
    ...
    byGender: {
      male: {
        ...
        normal: mockWeightMetrics.filter(
          (m) => m.gender === "masculino" && m.bmiCategory === "normal"
        ).length,
        overweight: mockWeightMetrics.filter(
          (m) => m.gender === "masculino" && m.bmiCategory === "sobrepeso"
        ).length,
        obesity: mockWeightMetrics.filter(
          (m) => m.gender === "masculino" && m.bmiCategory === "obesidad"
        ).length,
      },
      female: {
        total: mockStudents.filter((s) => s.gender === "femenino").length,
        normal: mockWeightMetrics.filter(
          (m) => m.gender === "femenino" && m.bmiCategory === "normal"
        ).length,
        overweight: mockWeightMetrics.filter(
          (m) => m.gender === "femenino" && m.bmiCategory === "sobrepeso"
        ).length,
        obesity: mockWeightMetrics.filter(
          (m) => m.gender === "femenino" && m.bmiCategory === "obesidad"
        ).length,
      },
    },
    byAgeGroup: {
      "10-12": {
        total: mockWeightMetrics.filter((m) => m.age >= 10 && m.age <= 12)
          .length,
        normal: mockWeightMetrics.filter(
          (m) => m.age >= 10 && m.age <= 12 && m.bmiCategory === "normal"
        ).length,
        overweight: mockWeightMetrics.filter(
          (m) => m.age >= 10 && m.age <= 12 && m.bmiCategory === "sobrepeso"
        ).length,
        obesity: mockWeightMetrics.filter(
          (m) => m.age >= 10 && m.age <= 12 && m.bmiCategory === "obesidad"
        ).length,
      },
      "13-15": {
        total: mockWeightMetrics.filter((m) => m.age >= 13 && m.age <= 15)
          .length,
        normal: mockWeightMetrics.filter(
          (m) => m.age >= 13 && m.age <= 15 && m.bmiCategory === "normal"
        ).length,
        overweight: mockWeightMetrics.filter(
          (m) => m.age >= 13 && m.age <= 15 && m.bmiCategory === "sobrepeso"
        ).length,
        obesity: mockWeightMetrics.filter(
          (m) => m.age >= 13 && m.age <= 15 && m.bmiCategory === "obesidad"
        ).length,
      },
      "16-18": {
        total: mockWeightMetrics.filter((m) => m.age >= 16 && m.age <= 18)
          .length,
        normal: mockWeightMetrics.filter(
          (m) => m.age >= 16 && m.age <= 18 && m.bmiCategory === "normal"
        ).length,
        overweight: mockWeightMetrics.filter(
          (m) => m.age >= 16 && m.age <= 18 && m.bmiCategory === "sobrepeso"
        ).length,
        obesity: mockWeightMetrics.filter(
          (m) => m.age >= 16 && m.age <= 18 && m.bmiCategory === "obesidad"
        ).length,
      },
    },
